...
export class CameraOperator implements IInputReceiver, IUpdatable {
  ...
  public handleKeyboardEvent(
    ...
    if (code === "KeyC" && pressed === true && event.shiftKey === true) {
      if (this.vehicleCaller !== undefined) {
        this.world.inputManager.setInputReceiver(this.vehicleCaller);
        this.vehicleCaller = undefined;
      }
    } else {
  ...
  // Helper method for setting up car camera position
  public setCarCameraPosition(): void {
    // Position the camera behind and slightly above the car
    this.phi = 15; // Look slightly downward
    this.setRadius(5, true); // Set distance from car
  }
}
