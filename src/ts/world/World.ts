...
export class World {
  ...
  public updatePhysics(timeStep: number): void {
    ...
    this.cars.forEach((car) => {
      car.physicsPreStep(car.collision);
    });

    // Step the physics world
    this.physicsWorld.step(this.physicsFrameTime, timeStep);

    // Post-step and bounds checking for cars
    this.cars.forEach((car) => {
      if (this.isOutOfBounds(car.rayCastVehicle.chassisBody.position)) {
        let worldPos = new THREE.Vector3();
        car.spawnPoint.getWorldPosition(worldPos);
        let worldPos_CANNON = new CANNON.Vec3(
          worldPos.x,
          worldPos.y + 1,
          worldPos.z
        );
        this.outOfBoundsRespawn(
          car.rayCastVehicle.chassisBody,
          worldPos_CANNON
        );
      }
    });
